...
def algorithm_4(drivers: List[Driver],rider : Rider,graph : Graph):
	...
	for d in drivers:
		...
		if m_d_org == m_r_org and s_r_org in d.get_trajectory().node_id_list :
			...
			if wd + w_chap_d <= 2.5  and  wt + w_chap_t <= 45  and  t_chap < t_first: # les longueurs en Mètres et le temps en Secondes
				...
				arrival_time_mp = t_prime + walk(r_org,graph.get_node(m_d_org),graph,5/60) # arrivée à m_r_org
				...
				rider.get_trajectory().update_trajectory(Foot(ID=m_d_org,Speed=5/60),arrival_time_mp,departure_time_mp,m_d_org) # ajouter m_d_org node à trajectory
		...
		for d in drivers:
			...
			if m_d_dst == m_prime and m_prime in d.get_trajectory().node_id_list and m_d_org in d.get_trajectory().node_id_list:
				...
				if w_chap_d <= 2.5  and  w_chap_t <= 45  and  t_chap < t_first: # les longueurs en Mètres et le temps en Secondes
					...
					rider.get_trajectory().update_trajectory(Foot(ID=m_d_org,Speed=5/60),arrival_time_mp,departure_time_mp,m_d_org) # ajouter m_d_org node à trajectory de r
					...
					rider.get_trajectory().update_trajectory(Foot(ID=s_r_org,Speed=5/60),arrival_time_station,departure_time_station,s_r_org) # ajouter s_r_org à trajectory de r
	...
	if t_first == np.Infinity:
		...
		if w_chap_d < 2.5 :
			...
			arrival_time_station = t_prime + walk(r_org,graph.get_node(s_r_org),graph,5/60) + 1 #arrivée à la plateforme de s_r_org
	...
	for d in drivers:
		...
		if m_d_dest == m_r_dest and s_r_dest in d.get_trajectory().node_id_list :
			...
			if wd + w_chap_d < 2.5 and wt+ w_chap_t <45 and t_chap<t_last:
				...
				arrival_time_dst = departure_time_m_r_dest + walk(graph.get_node(m_d_dest),r_dst,graph,5/60)
...
##--------------- RIDER HAS TO WALK TO A MEETING POINT IN LAST MILE-----------------------------------------
			...
			if m_d_dest == m_r_dest and s_r_dest in d.get_trajectory().node_id_list :
				if m_d_org == m_seconde :	
					...
					if wd + w_chap_d <= 2.5  and  wt + w_chap_t <= 45  and  t_chap < t_last: # les longueurs en Mètres et le temps en Secondes
						...
						arrival_time_m_seconde = t_prime + walk(graph.get_node(s_r_dest),graph.get_node(m_seconde),graph,5/60) # arrivée en m_seconde
						departure_time_m_seconde = arrival_time_m_seconde + wt	# départ de m_seconde
						arrival_time_m_r_dest = departure_time_m_seconde + Drive(graph.get_node(m_r_dest),graph.get_node(m_seconde),graph,40/60) # arrivée en m_r_dest
						departure_time_m_r_dest = departure_time_m_r_dest # départ de m_r_dest
						arrival_time_r_dst = departure_time_m_r_dest + walk(graph.get_node(m_r_dest),r_dst,graph,5/60) # arrivée en r_dest
						departure_time_r_dst = arrival_time_r_dst # départ de r_dest

						rider.get_trajectory().update_trajectory(Foot(ID=m_seconde,Speed=5/60),arrival_time_m_seconde,departure_time_m_seconde,m_seconde) # ajouter m_seconde à trajectory de r
						...
						rider.get_trajectory().update_trajectory(Foot(ID=r_dst.get_id(),Speed=5/60),arrival_time_r_dst,departure_time_r_dst,r_dst.get_id())  # ajouter m_dst à trajectory de r
	...
	if t_last == np.Infinity:
		...
		if w_chap_d < 2.5 :
			...
			rider.get_trajectory().update_trajectory(Foot(ID=r_dst.get_id(),Speed=5/60),arrival_time_r_dst,departure_time_r_dst,r_dst.get_id()) # ajouter r_dst à trajectory
			# mise à jour des informations du rider
			rider.update_walking_distance(wd)
