...
def algorithm_3(drivers : List[Driver],rider : Rider,graph : Graph):
    ...
    '''
    arrival_time_m_r_org = rider.born_time + wt + walk(r_org,graph.get_node(m_r_org),graph,rider.walking_speed/60) # arrivée à m_r_org
    ...
    rider.get_trajectory().update_trajectory(Foot(ID = r_dest.get_id(),Speed=rider.walking_speed),arrival_time_r_dest,departure_time_r_dest,r_dest.get_id())
