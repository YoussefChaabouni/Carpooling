...
def proportion_served_calculator_integrated_system(nb_riders, setOfPersons, setOfDrivers, simulation_graph, setOfTrains):
    proportion_served=0
    #print(len(setOfPersons.carpooler_dict ))

    for i in range(nb_riders):
        rider=setOfPersons.get_carpooler(f'rider{i}')[0]
        arrival_time=driver_selection_in_integrated_system(rider, setOfDrivers, setOfTrains, simulation_graph)

        if  arrival_time<math.inf:
            proportion_served+=1
            #print(rider.name, 'is served')
...
setOfPersons=persons_generator(nb_riders_min, nb_drivers_min, simulation_graph,setOfOrigins, setOfDestinations)[1]
List_nb_riders=[i for i in range(nb_riders_min,nb_riders_max,step)]
setOfCars=persons_generator(nb_riders_min, nb_drivers_min, simulation_graph,setOfOrigins, setOfDestinations)[2]
### proportion servered in integrated system ###
setOfDrivers=persons_generator(nb_riders_min, nb_drivers_max, simulation_graph,setOfOrigins, setOfDestinations)[0]
List_proportion_served_integrated=[]

for i in range(nb_riders_min,nb_riders_max, step):
    set2=persons_generator(i, nb_drivers_max, simulation_graph,setOfOrigins, setOfDestinations)[1]
    List_proportion_served_integrated+=[proportion_served_calculator_integrated_system(i,set2, setOfDrivers, simulation_graph, setOfTrains)]
    print(List_proportion_served_integrated[-1], i)


plt.plot(List_nb_riders, List_proportion_served_integrated)
